...
 
  ll maxOdd = -INF, minOdd = INF;   
  ll maxEven = -INF, minEven = INF; 
 
  for(int i=0;i<n;i++){
    if(i % 2 == 0){
      maxOdd = max(maxOdd, (ll)v[i]);
      minOdd = min(minOdd, (ll)v[i]);
    } else {
      maxEven = max(maxEven, (ll)v[i]);
      minEven = min(minEven, (ll)v[i]);
    }
  }
 
  
  bool ok = (minOdd - maxEven >= 2);
 
  cout << (ok ? "YES" : "NO") << endl;
}
 
